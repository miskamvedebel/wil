...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        if self.room.isPositionInRoom(self.position) == False:
            while True:
                direction = random.uniform(0, 360)
                if direction != self.direction:
                    new_position = self.prev_position.getNewPosition(direction, self.speed)
                    if self.room.isPositionInRoom(new_position):
                        break
            self.position = new_position
            self.direction = direction
        self.room.cleanTileAtPosition(self.prev_position)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        robots = {f'robot_{ii}': robot_type(room=room, speed=speed) for ii in range(num_robots)}
...
showPlot1(title="Time spent by number of robots", x_label="Number of robots", y_label="Avg Time Spent")
...
showPlot2(title="Time by aspect ratio", x_label="Aspect Ratios", y_label="Avg Time Spent")
